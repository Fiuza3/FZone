          ...
          <v-card color="warning" theme="dark" elevation="4" height="120">
            <v-card-text class="pa-4">
              <div class="d-flex justify-space-between align-center">
                ...
                <v-badge :content="reportData.lowStockItems.length" color="error">
              ...
              <v-progress-linear
                :model-value="(reportData.lowStockItems.length / reportData.totalProducts) * 100"
                color="white"
                height="4"
                rounded
                class="mt-2"
        ...
        >
          <template v-slot:item.name="{ item, index }">
            <div class="d-flex align-center py-2">
              <v-avatar :color="index < 3 ? 'success' : 'primary'" class="me-3" size="32">
                <span class="font-weight-bold">{{ index + 1 }}</span>
              </v-avatar>
              <div>
                <div class="font-weight-medium">{{ item.name }}</div>
                <v-chip :color="getCategoryColor(item.category)" size="x-small" variant="tonal">
                  {{ item.category }}
                </v-chip>
              </div>
            </div>
          ...
          <template v-slot:item.profit="{ item }">
            <span class="font-weight-bold text-success">{{ formatCurrency(item.profit) }}</span>
          </template>
          
          <template v-slot:item.profitMargin="{ item }">
            <v-chip :color="item.profitMargin > 30 ? 'success' : item.profitMargin > 15 ? 'warning' : 'error'" size="small">
          ...
          <v-card elevation="4">
            ...
            >
              ...
              <template v-slot:item.price="{ item }">
                <span class="font-weight-bold text-error">{{ formatCurrency(item.price) }}</span>
              </template>
              
              <template v-slot:item.cost="{ item }">
                <span class="text-medium-emphasis">{{ formatCurrency(item.cost) }}</span>
              </template>
            </v-data-table>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="6">
          <v-card elevation="4">
            ...
            >
              ...
              <template v-slot:item.price="{ item }">
                <span class="font-weight-bold text-success">{{ formatCurrency(item.price) }}</span>
              </template>
        ...
        <v-col cols="12" md="6">
          <v-card elevation="4">
            ...
            >
              ...
              <template v-slot:item.profitMargin="{ item }">
                <v-chip :color="item.profitMargin < 10 ? 'error' : 'warning'" size="small">
                  {{ formatPercentage(item.profitMargin) }}
                </v-chip>
              </template>
